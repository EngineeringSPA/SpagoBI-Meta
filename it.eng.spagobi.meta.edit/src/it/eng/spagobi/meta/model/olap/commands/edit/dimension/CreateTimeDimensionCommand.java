 ...
 * 
 */
public class CreateTimeDimensionCommand extends AbstractSpagoBIModelEditCommand {
	...
	private static Logger logger = LoggerFactory.getLogger(CreateTimeDimensionCommand.class);
	 ...
	 * @param parameter
	 */
	public CreateTimeDimensionCommand(EditingDomain domain, CommandParameter parameter) {
		super("model.olap.commands.edit.timedimension.create.label", "model.olap.commands.edit.timedimension.create.description",
				"model.olap.commands.edit.dimension.create", domain, parameter);
		olapModelInitializer = new OlapModelInitializer();
	}

	public CreateTimeDimensionCommand(EditingDomain domain) {
		this(domain, null);
	}
	...
	public void execute() {
		if (parameter.getValue() instanceof BusinessColumnSet) {
			...
			businessColumnSet.getProperties().get("structural.tabletype").setValue("time dimension");

			this.executed = true;
			logger.debug("Command [{}] executed succesfully", CreateTimeDimensionCommand.class.getName());
