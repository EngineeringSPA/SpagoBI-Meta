...
public class OlapModelManager {
	...
	/*
	 * Check if the selected Business Object on the Business Model could be related to the Olap Model, then check propertyId and value to determine if required
	 * to create Olap Objects
	 */
	public void checkForOlapModel(EditingDomain editingDomain, Object selectedBusinessObject, Object propertyId, Object value, String previousValue) {
		...
		if (selectedBusinessObject instanceof BusinessTable) {
			...
			// propertyId ="structural.tabletype", value="temporal dimension"
			else if ((propertyId.equals("structural.tabletype")) && (value.equals("temporal dimension"))) {
				createOlapTemporalDimension((BusinessColumnSet) selectedBusinessObject, previousValue);
			} else if ((propertyId.equals("structural.tabletype")) && (value.equals("time dimension"))) {
				createOlapTimeDimension((BusinessColumnSet) selectedBusinessObject, previousValue);
			}
	...
	// Create Olap Temporal Dimension via CreateTemporalDimensionCommand
	private void createOlapTemporalDimension(BusinessColumnSet businessColumnSet, String previousValue) {
		Model rootModel = businessColumnSet.getModel().getParentModel();
		OlapModel olapModel = rootModel.getOlapModels().get(0);

		CommandParameter commandParameter = new CommandParameter(olapModel, previousValue, businessColumnSet, new ArrayList<Object>());
		if (editingDomain != null) {
			editingDomain.getCommandStack().execute(new CreateTemporalDimensionCommand(editingDomain, commandParameter));
		}
	}

	// Create Olap Time Dimension via CreateTimeDimensionCommand
	private void createOlapTimeDimension(BusinessColumnSet businessColumnSet, String previousValue) {
		Model rootModel = businessColumnSet.getModel().getParentModel();
		OlapModel olapModel = rootModel.getOlapModels().get(0);
