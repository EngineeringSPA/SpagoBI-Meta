...
public class HierarchiesEditorMainPage extends Dialog {
	...
	private static Logger logger = LoggerFactory.getLogger(HierarchiesEditorMainPage.class);
	 ...
	 * @param parentShell
	 */
	public HierarchiesEditorMainPage(Shell parentShell, OlapModel olapModel, BusinessColumnSet businessColumnSet, String projectName) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		btnAddHierarchy.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				try {
					hierarchyEditor = new HierarchyEditor(getShell(), olapModel, businessColumnSet, olapModelInitializer, projectName);
				...
				} catch (NoActiveServerException ex) {
					logger.error("No server is defined active");

					MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error",
							"No active server found. Please define one active server in order to retrieve level types values.");
				} catch (RemoteException ex) {
					logger.error("Cannot retrieve level types values from server");
					MessageDialog.openError(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), "Error",
							"Cannot retrieve level types values from server.Please check if the server is up and the connection is working.");
				}
		...
		/*
		 * TableColumn tblclmnPrimaryKey = new TableColumn(tableHierarchies, SWT.NONE); tblclmnPrimaryKey.setWidth(100);
	...
	private void populateTableHierarchies() {
		...
		if (!hierarchies.isEmpty()) {
			for (Hierarchy hierarchy : hierarchies) {
				...
				hyerarchyDescriptor.setHasAll(Boolean.valueOf(hierarchy.getProperties().get(OlapModelPropertiesFromFileInitializer.HIERARCHY_HAS_ALL)
						.getValue()));
				if (hierarchy.getProperties().get(OlapModelPropertiesFromFileInitializer.HIERARCHY_IS_DEFAULT) != null) {
					hyerarchyDescriptor.setDefaultHierarchy(Boolean.valueOf(hierarchy.getProperties()
							.get(OlapModelPropertiesFromFileInitializer.HIERARCHY_IS_DEFAULT).getValue()));
				} else {
				...
				hyerarchyDescriptor
						.setAllMemberName(hierarchy.getProperties().get(OlapModelPropertiesFromFileInitializer.HIERARCHY_ALL_MEMBER_NAME).getValue());
				...
				if (!hierarchyLevels.isEmpty()) {
					for (Level level : hierarchyLevels) {
						...
						hierarchyLevelDescriptor.setUniqueMembers(Boolean.valueOf(level.getProperties()
								.get(OlapModelPropertiesFromFileInitializer.LEVEL_UNIQUE_MEMBERS).getValue()));

						hierarchyLevelDescriptor.setLevelType(level.getProperties().get(OlapModelPropertiesFromFileInitializer.LEVEL_TYPE).getValue());
		...
		buttonEdit.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (index >= 0) {
					...
					HierarchyEditor hierarchyEditor;
					try {
						hierarchyEditor = new HierarchyEditor(getShell(), olapModel, businessColumnSet, olapModelInitializer, hierarchyDescriptor, projectName);
						hierarchyEditor.create();
						if (hierarchyEditor.open() == Window.OK) {
							HierarchyDescriptor hyerarchyDescriptor = hierarchyEditor.getHyerarchyDescriptor();
	...
	/**
	 * @param hierarchiesDescriptors
	 *            the hierarchiesDescriptors to set
	 */
	public void setHierarchiesDescriptors(List<HierarchyDescriptor> hierarchiesDescriptors) {
