...
public class AddBusinessRelationshipWizardPage extends WizardPage {
	private int cardinality = MANY_TO_ONE; // initial default value
	...
	private final BusinessColumnSet sourceTable;
	private final BusinessColumnSet destinationTable;
	...
	private final java.util.List<String> sourceColumnUniqueNamesList, destinationColumnUniqueNamesList, sourceTableUniqueNamesList,
			destinationTableUniqueNamesList;
	private boolean nameInserted;

	private Button button1to1;

	private Button button1toN;

	private Button buttonNto1;

	private Button button1optTo1;

	private Button button1ToOpt1;

	private Button buttonOpt1ToN;

	private Button button1ToOptN;

	private Button buttonOptNto1;

	private Button buttonNtoOpt1;
	...
	public void initCardinalityGroup(Composite parent, int style) {
		...
		button1to1 = new Button(compGroup, SWT.RADIO);
		...
		button1toN = new Button(compGroup, SWT.RADIO);
		...
		buttonNto1 = new Button(compGroup, SWT.RADIO);
		buttonNto1.setText("N to 1");
		buttonNto1.setSelection(true);
		...
		/*
		 * Button buttonNtoN = new Button(compGroup, SWT.RADIO); buttonNtoN.setText("N to N"); buttonNtoN.addSelectionListener(new SelectionAdapter() {
		 * 
		 * @Override public void widgetSelected(SelectionEvent e) { cardinality = MANY_TO_MANY; } });
		 */

		button1optTo1 = new Button(compGroup, SWT.RADIO);
		...
		button1ToOpt1 = new Button(compGroup, SWT.RADIO);
		...
		buttonOpt1ToN = new Button(compGroup, SWT.RADIO);
		...
		button1ToOptN = new Button(compGroup, SWT.RADIO);
		...
		buttonOptNto1 = new Button(compGroup, SWT.RADIO);
		...
		buttonNtoOpt1 = new Button(compGroup, SWT.RADIO);
		...
		btnAddRelationship.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				if ((sourceColumnList.getSelection().length != 0) && (destinationColumnList.getSelection().length != 0)
						&& (getBusinessRelationshipName().length() != 0)) {
					setErrorMessage(null);
					columnCorrelationList.add(sourceTableCombo.getText() + "." + sourceColumnList.getSelection()[0] + " -> " + destinationTableCombo.getText()
							+ "." + destinationColumnList.getSelection()[0]);
	...
	private String cardinalityToString(int cardinality) {
		...
		case 4:
			return "many-to-many";
		case 5:
			return "optional-one-to-one";
		case 6:
			return "one-to-optional-one";
		case 7:
			return "optional-one-to-many";
		case 8:
			return "one-to-optional-many";
		case 9:
			return "optional-many-to-one";
		case 10:
			return "many-to-optional-one";
		default:
			return "one-to-many";
		}
	}
	...
	public BusinessRelationshipDescriptor getRelationshipDescriptor() {
		return new BusinessRelationshipDescriptor(getSourceTable(), getDestinationTable(), sourceColumns, destinationColumns, cardinalityToString(cardinality),
				getBusinessRelationshipName());
	}
}
