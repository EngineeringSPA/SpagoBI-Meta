 ...
 *
 */
public class PhysicalModelEditorCommandStackListener implements CommandStackListener {
	
	PhysicalModelEditor editor;
	
	public PhysicalModelEditorCommandStackListener(PhysicalModelEditor editor) {
		this.editor = editor;
	}
			...
			(new Runnable() {
				public void run() {
					...
					if (editor.getPropertySheetPage() != null && editor.getPropertySheetPage().getControl()!=null && !editor.getPropertySheetPage().getControl().isDisposed()) {
						//TODO: MC, implement property sheet for Physical Model
				...
				public void validateCommandResults(Command mostRecentCommand, boolean undoOnError) {
					...
					if(it.hasNext()) {
						...
						if(model != null) {
							...
							if (!(mostRecentCommand instanceof UpdatePhysicalModelCommand)){
